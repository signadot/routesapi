...
func (c *routesClient) GetClusterServices(ctx context.Context, in *GetClusterServicesRequest, opts ...grpc.CallOption) (*GetClusterServicesResponse, error) {
	out := new(GetClusterServicesResponse)
	err := c.cc.Invoke(ctx, "/routes.Routes/GetClusterServices", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (UnimplementedRoutesServer) GetClusterServices(context.Context, *GetClusterServicesRequest) (*GetClusterServicesResponse, error) {
	return nil, status.Errorf(codes.Unimplemented, "method GetClusterServices not implemented")
}
...
func _Routes_GetClusterServices_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(GetClusterServicesRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(RoutesServer).GetClusterServices(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/routes.Routes/GetClusterServices",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(RoutesServer).GetClusterServices(ctx, req.(*GetClusterServicesRequest))
	}
	return interceptor(ctx, in, info, handler)
}
